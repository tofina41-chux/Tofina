...
export async function POST(request) {
  ...
  try {
    ...
    await supabaseAdmin.from('profile').upsert({ id: 1, imageUrl: publicUrlData.publicUrl });

    return new Response(JSON.stringify({ success: true, imageUrl: publicUrlData.publicUrl }), { status: 200 });
  } catch (err) {
