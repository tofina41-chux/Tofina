...
- `ADMIN_SECRET_KEY` — secret key used by admin API calls for uploads and project creation
- `VITE_API_URL` — public API base URL (e.g. `https://your-vercel-deployment.vercel.app`). In production this can be omitted if the frontend and API are served from the same Vercel domain.
